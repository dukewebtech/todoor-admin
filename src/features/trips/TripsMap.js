...
function TripsMap({ route , width}) {
    ...
    const [directionResponse, setDirectionResponse] = useState(/** @type google.maps.map*/(null))
    ...
    return isLoaded ? (
        ...
        {width&&<div class="flex w-1/3  items-center mr-5">
            <Typography className='w-full text-green-600' variant='h6'>{route.origin}</Typography>
            <AiOutlineArrowRight style={{ fontSize:"40px"}}/>
            <Typography className='w-full text-green-600 ml-5' variant='h6'>{route.destination}</Typography>
        </div> }
